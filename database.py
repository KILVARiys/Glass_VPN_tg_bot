...
def create_user(telegram_id, username, first_name, referrer_id=None):
    ...
    cur.execute('''
        INSERT INTO users (telegram_id, username, first_name, xui_email, is_trial_used, referrer_id)
        VALUES (?, ?, ?, ?, 0, ?)
    ''', (telegram_id, username, first_name, xui_email, referrer_id))
    conn.commit()
    conn.close()
    if referrer_id:
...
def update_subscription(telegram_id, days_to_add):
    ...
    if result and result[0]:
        ...
        new_end = datetime.now() + timedelta(days=days_to_add)
    cur.execute("UPDATE users SET subscription_end = ? WHERE telegram_id = ?", (new_end.isoformat(), telegram_id))
    conn.commit()
    conn.close()
    return new_end
...
def set_trial_used(telegram_id):
    """Устанавливает флаг, что пробный период использован"""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    ...
    import logging
    logging.getLogger(__name__).info(f"✅ set_trial_used: пользователь {telegram_id}, обновлено строк: {affected}")
    return affected > 0
...
def use_promocode(telegram_id, code):
    ...
    conn.commit()
    conn.close()
    return True, f"Промокод активирован! Добавлено {bonus_days} дней."
